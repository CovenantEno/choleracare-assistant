...
import { sampleConsultations, sampleNotifications } from '@/lib/sample-data';
import RiskBadge from '@/components/RiskBadge';

export default function PatientDashboard() {
  const { user } = useAuth();

  const myConsultations = sampleConsultations.slice(0, 3);
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Consultations" value={3} icon={Activity} color="teal" />
        <StatCard title="Last Risk Level" value="Moderate" icon={Shield} color="orange" />
        <StatCard title="Days Since Check" value={2} icon={Heart} color="green" />
        <StatCard title="Prevention Score" value="85%" icon={Droplets} color="blue" />
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-card rounded-xl border border-border p-5 shadow-card">
            ...
            <div className="mt-3 h-2 bg-muted rounded-full overflow-hidden">
              <div className="h-full bg-gradient-hero rounded-full" style={{ width: '65%' }} />
            </div>
            <p className="text-xs text-muted-foreground mt-1.5">5 of 8 glasses today</p>
          </div>
            ...
            <ul className="space-y-2.5">
              {['Boil or treat drinking water', 'Wash hands before eating', 'Avoid raw street food', 'Use sanitary facilities'].map(tip => (
                <li key={tip} className="flex items-start gap-2 text-sm text-muted-foreground">
                  <Shield className="w-4 h-4 text-medical-green mt-0.5 shrink-0" />
                  {tip}
                </li>
              ))}
            </ul>
